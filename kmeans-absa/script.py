...
def findCategory(keyword):
    for key, val in serviceToKeyword.items():
        if keyword in val:
            return key
    return 'No Servicing Category Detected'
...
def main():
    print("Introducing K-means Ngrams Aspect Based Sentiment Analysis")
    print("-"*75)
    ...
    while(True):
        ...
        if len(ngrams) == 0:
            print("No servicing category detected!")
        for key, val in ngrams.items():
            prediction = kmeansPrediction(key,val)
            if (prediction[0] == 'No Servicing Category Detected'):
                print('No Servicing Category Detected!')
            else:
                if prediction[1] == '-1':
