...
def evalLSTM(sentence, services, senti):
    correct_count = 0
    #cleaned_input = cleanInput(sentence)
    #sentence, k = extractSentenceAndKeywords(cleaned_input)
    #for key in k: 
        #category = findCategory(key)
    sentiment = infer_example.main(sentence, '$T$', './lstm-atae/')
    print("REAL SENTIMENT: ", senti)
    print("DETERMINED SENTIMENT: ", sentiment[0])
    if int(sentiment[0]) == int(senti): #and category in services:
        correct_count += 1
    return correct_count
...
def main():
    ...
    #Read in the test data category
    l = open('./lstm-atae/datasets/semeval14/Car_Test_Gold.xml.seg', 'r')
    line_count = 0
    comments = []
    keywords = []
    sentiments = []
    ...
    #print(comments, keywords, sentiments)
    total = 0
    kmeans_correct = 0
    lstm_correct = 0
    #cabasc_correct = 0
